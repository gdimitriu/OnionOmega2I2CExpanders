...
void Adafruit_PWMServoDriver::setPWM(uint8_t t_num, uint16_t t_on, uint16_t t_off) {
  ...
  uint8_t buffer[1];
  buffer[0] = t_on;
  i2c_writeBuffer(0, m_i2caddr, PCA9685_LED0_ON_L + 4 * t_num, buffer, 1);
  buffer[0] = 0;
  buffer[0] = t_on >> 8;
  i2c_writeBuffer(0, m_i2caddr, PCA9685_LED0_ON_L + 4 * t_num + 1, buffer, 1);
  buffer[0] = 0;
  buffer[0] = t_off;
  i2c_writeBuffer(0, m_i2caddr, PCA9685_LED0_ON_L + 4 * t_num + 2, buffer, 1);
  buffer[0] = 0;
  buffer[0] = t_off >> 8;
  i2c_writeBuffer(0, m_i2caddr, PCA9685_LED0_ON_L + 4 * t_num + 3, buffer, 1);
}
